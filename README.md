...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create fireworks-d5d2
  fly apps create fireworks-d5d2-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app fireworks-d5d2
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app fireworks-d5d2-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'fireworks-d5d2-staging' does not match app name in config file 'fireworks-d5d2'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name fireworks-d5d2-db
  fly postgres attach --app fireworks-d5d2 fireworks-d5d2-db

  fly postgres create --name fireworks-d5d2-staging-db
  fly postgres attach --app fireworks-d5d2-staging fireworks-d5d2-staging-db
  ```
